					...
					<div class="Table_LBx">
						<div class="Table_left" style="overflow-x:hidden;;height:235px">
							<table>
								...
								<tr>
									<th>하수맨홀용도</th>
									<td><select name="SMU_CDE" id="SMU_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${smu_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.smuCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
				                    <th>하수맨홀종류</th>
									<td><select name="SOM_CDE" id="SOM_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${som_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.somCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>				                    
								</tr>
								<tr>
				                    ...
				                    <th>최종준설일자</th>
									<td><input type="text" name="ECN_YMD" id="ECN_YMD"  value='<c:out value="${result.ecnYmd}"/>' class="input DT_DATE" /></td>
								</tr>
								<tr>
									<th>하수맨홀구경</th>
									<td><input type="text" name="MAN_DIP" id="MAN_DIP" value='<c:out value="${result.manDip}"/>' class="input MX_4 CS_4 DT_INT" /> mm</td>
									<th>하수맨홀가로</th>
									<td><input type="text" name="MAN_HOL" id="MAN_HOL" value='<c:out value="${result.manHol}"/>' class="input MX_5 DT_FLOAT DD_2" /> </td>									
								</tr>
								<tr>
									<th>하수맨홀뚜껑재질</th>
									<td><select name="SBC_CDE" id="SBC_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${sbc_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sbcCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
									<th>하수맨홀세로</th>
									<td><input type="text" name="MAN_VEL" id="MAN_VEL" value='<c:out value="${result.manVel}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>									
								</tr>	
								<tr>
									<th>인버트유무</th>
									<td><select name="IVT_CDE" id="IVT_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${ivt_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.ivtCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
									<th>하수맨홀고도</th>
									<td><input type="text" name="MOS_HSL" id="MOS_HSL" value='<c:out value="${result.mosHsl}"/>' class="input MX_7 DT_FLOAT DD_2" /> m</td>
								</tr>	
								<tr>
									<th>사다리설치유무</th>
									<td><select name="LAD_CDE" id="LAD_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${lad_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.ladCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
									<th>하수맨홀저고</th>
									<td><input type="text" name="LMS_HSL" id="LMS_HSL" value='<c:out value="${result.lmsHsl}"/>' class="input MX_7 DT_FLOAT DD_2" /> m</td>									
								</tr>										
								<tr>
									<th>하수맨홀이상상태</th>
									<td colspan="3"><select name="CST_CDE" id="CST_CDE" class="select">
		                            <option value=""></option>
