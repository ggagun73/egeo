		...
		<div id="content2">
			...
			<div class="TabArea">
				...
				<div id="_Tab1" class="PSection on">
					<div class="Table_LBx">
						<div class="Table_left" style="height:170px">
							<table>
								<colgroup>
									<col width="19%" />
									<col width="31%" />
					...
					<div class="Table_LBx">
						<div class="Table_left" style="height:170px">
							<table>
								...
								<tr>
									<th>물받이용도</th>
									<td><select name="SBD_CDE" id="SBD_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${sbd_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sbdCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
				                    <th>최종준설일자</th>
									<td><input type="text" name="ECN_YMD" id="ECN_YMD" value="<c:out value="${result.ecnYmd}"/>" class="input DT_DATE" />
								</tr>
								<tr>
									<th>물받이형태</th>
				                    ...
				                    <th>원형물받이내경</th>
									<td><input type="text" name="SPT_DIP" id="SPT_DIP" value='<c:out value="${result.sptDip}"/>' class="input MX_4 CS_4 DT_INT" /> mm</td>
								</tr>
								<tr>
								 	<th>물받이뚜껑형태</th>
									<td><select name="COV_CDE" id="COV_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${cov_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.covCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
									<th>각형물받이가로길이</th>
									<td><input type="text" name="SPT_HOL" id="SPT_HOL" value='<c:out value="${result.sptHol}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>									
								</tr>
								<tr>
									<th>물받이뚜껑재질</th>
									...
									<th>각형물받이세로길이</th>
									<td><input type="text" name="SPT_VEL" id="SPT_VEL" value='<c:out value="${result.sptVel}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>									
								</tr>	
								<tr>
									<th>물받이깊이</th>
									<td colspan=3><input type="text"  name="SPT_DEP" id="SPT_DEP" value='' class="input MX_5 DT_FLOAT DD_2"  value="<c:out value="${result.sptDep}"/>"/>m</td>
								</tr>								
							</table>
						</div>
					</div>
