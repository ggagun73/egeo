					...
					<div class="Table_LBx">
						<div class="Table_left">
							<table>
								...
								<tr>								
									<th>처리분구명</th>
									<td><input type="text" name="DDP_NAM" id="DDP_NAM" value='<c:out value="${result.ddpNam}"/>' class="input MX_20" /></td>
									<th>처리분구인구수</th>
									<td><input type="text" name="DDP_POP" id="DDP_POP" value='<c:out value="${result.ddpPop}"/>' class="input MX_8 DT_INT" /> 인</td>
								</tr>
								<tr>
									 <th>처리분구면적</th>
									<td><input type="text" name="DDP_SIZ" id="DDP_SIZ" value='<c:out value="${result.ddpSiz}"/>' class="input MX_6 DT_INT" /> ㏊</td>
				                  	 <th>계획처리인구수</th>
									<td><input type="text" name="PRN_POP" id="PRN_POP" value='<c:out value="${result.prnPop}"/>' class="input MX_8 DT_INT" /> 인</td>				   			                    
								</tr>
								<tr>
									 <th>주거지역면적</th>
									<td><input type="text" name="RES_SIZ" id="RES_SIZ" value='<c:out value="${result.resSiz}"/>' class="input MX_6 DT_INT" /> ㏊</td>
				                     <th>오수발생량</th>
									<td><input type="text" name="SEW_VOL" id="SEW_VOL" value='<c:out value="${result.sewVol}"/>' class="input MX_8 DT_INT" /> ㎥/일</td>               
								</tr>
								<tr>	
									 <th>상업지역면적</th>
									<td><input type="text" name="COM_SIZ" id="COM_SIZ" value='<c:out value="${result.comSiz}"/>' class="input MX_6 DT_INT" /> ㏊</td>
									 <th>공업지역면적</th>
									<td><input type="text" name="IND_SIZ" id="IND_SIZ" value='<c:out value="${result.indSiz}"/>' class="input MX_6 DT_INT" /> ㏊</td>
								</tr>
								<tr>
									<th>대장초기화여부</th>
