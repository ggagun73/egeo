...
<%
  /**
  * @Class Name : 
  * @Description : RDL_STAT_PS 정류장 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","STT_YMD", 10); // 사용개시일자
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="STT_YMD" id="STT_YMD" value="<c:out value="${result.sttYmd}"/>" class="input DT_DATE" />
								</tr>
								<tr>
									<th>행정읍/면/동</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>           
				                    <th>정류장명</th>
									<td colspan="3"><input type="text" name="STT_NAM" id="STT_NAM" value='<c:out value="${result.sttNam}"/>'  style="width:400px" class="input MX_30 CS_30" /></td>		           
								</tr>				
								<tr>
									<th>정류장종류</th>
									<td><select name="AEE_CDE" id="AEE_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${aee_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.aeeCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	             
									<th>정류장유형</th>
									<td><select name="STT_CDE" id="STT_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${stt_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sttCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	                          
								</tr>		
							</table>
