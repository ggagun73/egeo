...
function fncAddAuthorGroupApprove(id, sys, author) {
    if(confirm("시스템 사용을 승인하시겠습니까?")) {
        document.listForm.userId.value = id;
        document.listForm.sysCode.value = sys;
        document.listForm.authorCode.value  = author;
    	document.listForm.action = "<c:url value='/sec/rgm/EgovAuthorGroupApprove.do'/>";
        document.listForm.submit();
    }
}
...
<table width="100%" cellpadding="8" class="table-search" border="0">
 <tr>
  ...
  <td width="50%">조회조건 : <select name="searchCondition" onchange="onSearchCondition()" title="조회조건선택">
                    ...
                    <option value="3" <c:if test="${authorGroupVO.searchCondition == '3'}">selected</c:if> >시스템명</option>
...
<%--      <table border="0" cellspacing="0" cellpadding="0">
  ...
  <th width="20%" align="right">
   <table border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_left.gif' />" width="8" height="20" alt="버튼이미지"></td>
      <td class="btnBackground" nowrap="nowrap"><a href="#" onclick="javascript:fncSelectAuthorGroupList('1')" style="selector-dummy:expression(this.hideFocus=false);">조회</a>
      </td>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
      <td>&nbsp;&nbsp;</td>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_left.gif'/>" width="8" height="20" alt="버튼이미지"></td>
      <td class="btnBackground" nowrap="nowrap"><a href="#" onclick="javascript:fncAddAuthorGroupInsert(); return false;" style="selector-dummy:expression(this.hideFocus=false);">일괄승인</a>
      </td>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
      <td>&nbsp;&nbsp;</td>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_left.gif'/>" width="8" height="20" alt="버튼이미지"></td>
      <td class="btnBackground" nowrap="nowrap"><a href="#" onclick="javascript:fncAuthorGroupDeleteList()" style="selector-dummy:expression(this.hideFocus=false);">삭제</a>
      </td>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
    </tr>
   </table>
...
<table width="100%" cellpadding="8" class="table-line" summary="권한 그룹을 관리하는 테이블입니다.사용자 ID,사용자 명,사용자 유형,권한,등록 여부의 정보를 담고 있습니다.">
 <thead>
  <tr>
    ...
    <th class="title" width="20%" scope="col" nowrap="nowrap">시스템명</th>
    <th class="title" width="10%" scope="col" nowrap="nowrap">신청일자</th>
    <th class="title" width="10%" scope="col" nowrap="nowrap">승인일자</th>
  </tr>
 </thead>
 <tbody>
 <c:forEach var="authorGroup" items="${authorGroupList}" varStatus="status">
  <tr>
    <td class="lt_text3" nowrap="nowrap"><input type="checkbox" name="delYn" class="check2" title="선택"><input type="hidden" name="checkId" value="<c:out value="${authorGroup.userId}"/>"/></td>
    <td class="lt_text3" nowrap="nowrap"><c:out value="${authorGroup.userId}"/></td>
    <td class="lt_text3" nowrap="nowrap"><c:out value="${authorGroup.userName}"/></td>
    <td class="lt_text3" nowrap="nowrap"><c:out value="${authorGroup.authorCode}"/></td>
   <%--  <td class="lt_text3" nowrap="nowrap"><select name="authorManageCombo" title="권한">
    ...
    <td class="lt_text3" nowrap="nowrap"><c:out value="${authorGroup.reqDate}"/></td>
    <td class="lt_text3" nowrap="nowrap">
    	<c:if test='${!empty authorGroup.approveDate}'><c:out value="${authorGroup.approveDate}"/></c:if>
    	<c:if test='${empty authorGroup.approveDate}'><a href="#" onclick="javascript:fncAddAuthorGroupApprove('<c:out value="${authorGroup.userId}"/>','<c:out value="${authorGroup.sysCode}"/>','<c:out value="${authorGroup.authorCode}"/>'); return false;" style="selector-dummy:expression(this.hideFocus=false);">승인</a></c:if>
    </td>    
  </tr>
 </c:forEach>
 </tbody>
