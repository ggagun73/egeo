	...
	<script type="text/javascript">
		...
		function fn_set_relateNum() {
			...
			if( sRowId != null ) {
				...
				if( sKeyColumn == "BOM_IDN" || sKeyColumn == "EOM_IDN"  || sKeyColumn == "MAN_IDN" ) {	//하수관거대장에서 시점, 종점 맨홀 
					var sKeyNum1 = sRowData["FTR_CDE"];	
					var sKeyNum2 = sRowData["FTR_IDN"];							
					var sPreName = sKeyColumn.substring(0,3);
					
					//alert(sPreName+"||"+sKeyNum1+"||"+sKeyNum2);
					
					$("#win_"+sOpenerId).find("#"+sPreName+"_CDE").val( sKeyNum1 );					
					$("#win_"+sOpenerId).find("#"+sPreName+"_IDN").val( sKeyNum2 );
					$("#win_"+sOpenerId).find("#"+sPreName+"_CDE option").not(":selected").attr("disabled", "disabled");
				}else if( sKeyColumn.indexOf("IDN") != -1 ) {		//토구대장에서 처리구역, 배수구역 검색.. 
					var sKeyNum = sRowData["FTR_IDN"];		
					//alert(sOpenerId+"||"+sKeyNum1+"||"+sKeyNum2);						
