...
<%
  /**
  * @Class Name : 
  * @Description : RDL_MDST_AS 중앙분리대관리 
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="RGR_YMD" id="RGR_YMD" value="<c:out value="${result.rgrYmd}"/>" class="input DT_DATE" />				                    
								</tr>
								<tr>
									<th>관리기관</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>종류</th>
									<td><select name="RGR_CDE" id="RGR_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${rgr_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.rgrCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
				                    <th>폭원</th>
									<td><input type="text" name="RGR_WID" id="RGR_WID" value='<c:out value="${result.rgrWid}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		          
								</tr>
								<tr>
									<th>연장</th>
									<td><input type="text" name="RGR_LEN" id="RGR_LEN" value='<c:out value="${result.rgrLen}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		
				                    <th>높이</th>
									<td><input type="text" name="RGR_HIT" id="RGR_HIT" value='<c:out value="${result.rgrHit}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		         
								</tr>							
							</table>
						</div>
					</div>
