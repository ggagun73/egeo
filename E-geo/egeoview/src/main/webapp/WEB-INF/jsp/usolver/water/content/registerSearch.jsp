	...
	<script type="text/javascript">
		...
		function fn_set_relateNum() {
			...
			if( sRowId != null ) {
				...
				//상수관로 검색일 경우 관로번호, 지형지물부호를 같이 넘기자.. 
				if( sKeyColumn == "PIP_IDN") {
					var sKeyNum1 = sRowData["FTR_CDE"];	
					var sKeyNum2 = sRowData["FTR_IDN"];		
					//alert(sOpenerId+"||"+sKeyNum1+"||"+sKeyNum2);
					
					$("#win_"+sOpenerId).find("#PIP_CDE").val( sKeyNum1 );					
					$("#win_"+sOpenerId).find("#PIP_IDN").val( sKeyNum2 );
					$("#win_"+sOpenerId).find("#PIP_CDE option").not(":selected").attr("disabled", "disabled");
				}else {
