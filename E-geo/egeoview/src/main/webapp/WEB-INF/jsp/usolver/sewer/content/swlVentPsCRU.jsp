...
<div id="wrap2">
	<div id="container2"> 
		<div id="content2">
							...
							<table>
								...
								<tr>
									...
									<td colspan="3"><input type="text" name="CNT_NUM" id="CNT_NUM" value="<c:out value="${result.cntNum}"/>" class="input CS_10"  readonly />
                          									 <a href="#" onclick="BOOK.fn_open_cntNum('SWT_CONS_MA');"><img src="/images/usolver/com/book/btn_search.gif" border="0" class="vmiddle btnClassEdit"/></a>
						...
						<div class="Table_left" style="height:100px">
							<table>
								...
								<tr>
				                    ...
				                    <th>흡출기재질</th>
									<td><select name="HMP_CDE" id="HMP_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${hmp_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.hmpCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>				                    
								</tr>
								<tr>
									<th>환기구구경</th>
									<td><input type="text" name="VNT_DIP" id="VNT_DIP" value='<c:out value="${result.vntDip}"/>' class="input MX_4 DT_INT" /> mm</td>
				                    <th>흡출기형식</th>
			...
			<div class="Btn_R">
				<div class="Btn disBatchUp"><a href="#" class="Btn_02" onclick="BOOK.fn_view_consMa('SWT_CONS_MA');">공사대장</a></div>
