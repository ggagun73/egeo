...
<%
  /**
  * @Class Name : 
  * @Description : RDL_STLT_PS 가로등 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","LCD_YMD", 10); // 사용개시일자
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="LCD_YMD" id="LCD_YMD" value="<c:out value="${result.lcdYmd}"/>" class="input DT_DATE" />
								</tr>
								<tr>
									<th>행정읍/면/동</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>암길이</th>
									<td><input type="text" name="ARM_LEN" id="ARM_LEN" value='<c:out value="${result.armLen}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>	
				                    <th>등주높이</th>
									<td><input type="text" name="PLS_HIT" id="PLS_HIT" value='<c:out value="${result.plsHit}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		          
								</tr>
								<tr>
									<th>등기구수량</th>
									<td><input type="text" name="EDG_CNT" id="EDG_CNT" value='<c:out value="${result.edgCnt}"/>' class="input MX_5 DT_INT_2" /> 개</td>		          
									<th>등주형식</th>
									<td><select name="PLT_CDE" id="PLT_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${plt_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.pltCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>					                    
								</tr>
								<tr>
									<th>광원종류</th>
									<td><select name="LCD_CDE" id="LCD_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${lcd_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.lcdCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>등주재질</th>
									<td><select name="PLM_CDE" id="PLM_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${plm_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.plmCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>		         
								</tr>				
								<tr>
									<th>등기구모형</th>
									<td colspan="3"><select name="LSH_CDE" id="LSH_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${lsh_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.lshCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>					                         
								</tr>			
							</table>
