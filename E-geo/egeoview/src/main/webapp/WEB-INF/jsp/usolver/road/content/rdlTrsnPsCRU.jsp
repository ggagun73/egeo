...
<%
  /**
  * @Class Name : 
  * @Description : RDL_TRSN_PS 신호등 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","SGN_YMD", 10); // 사용개시일자
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="SGN_YMD" id="SGN_YMD" value="<c:out value="${result.sgnYmd}"/>" class="input DT_DATE" /></td>			
								</tr>
								<tr>
									<th>행정읍/면/동</th>
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
				                    ...
				                    <th>신호등종류</th>
									<td><select name="SGN_CDE" id="SGN_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${sgn_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sgnCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
								</tr>
								<tr>
									<th>신호등재질</th>
									<td><select name="SGM_CDE" id="SGM_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${sgm_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sgmCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>신호등수</th>
									<td><input type="text" name="SGN_CNT" id="SGN_CNT" value='<c:out value="${result.sgnCnt}"/>' class="input MX_2 DT_INT_2" /> 개</td>		         
								</tr>	
								<tr>
									<th>잔여시간표시기유무</th>
									<td><select name="LFT_CDE" id="LFT_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${lft_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.lftCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>신호등규격</th>
									<td><input type="text" name="SGN_EXP" id="SGN_EXP" value='<c:out value="${result.sgnExp}"/>' class="input MX_50 CS_50" /></td>		         
								</tr>													
							</table>
						</div>
					</div>
