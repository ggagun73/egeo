...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","BYC_YMD", 10); // 사용개시일자
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>자전거노선명</th>
									<td colspan="3"><input type="text" style="width:375px" name="BYC_NAM" id="BYC_NAM"  value='<c:out value="${result.bycNam}"/>' class="input MX_50 CS_50" /></td>			                    
								</tr>
								<tr>
									<th>관리기관</th>
								...
								<tr>
									<th>설치일자</th>
									<td><input type="text" name="BYC_YMD" id="BYC_YMD" value="<c:out value="${result.bycYmd}"/>" class="input DT_DATE" />
									<th>대장초기화여부</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>자전거도로구분</th>
									<td><select name="BYC_CDE" id="BYC_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${byc_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.bycCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
				                    <th>위치구분</th>
								...
								<tr>
									<th>자전거도로재질</th>
									<td><select name="BMT_CDE" id="BMT_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${bmt_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.bmtCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>경계종류</th>
									<td><select name="DWY_CDE" id="DWY_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${dwy_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.dwyCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	                    
								</tr>
								<tr>
									<th>폭원</th>
									<td><input type="text" name="BYC_WID" id="BYC_WID" value='<c:out value="${result.bycWid}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		
				                    <th>연장</th>
									<td><input type="text" name="BYC_LEN" id="BYC_LEN" value='<c:out value="${result.bycLen}"/>' class="input MX_5 DT_FLOAT DD_2" />  m</td>				                    
								</tr>
								<tr>
									<th>국비</th>
									<td><input type="text" name="NAT_AMT" id="NAT_AMT" value='<c:out value="${result.natAmt}"/>' class="input MX_11 DT_INT" /> 원</td>		
				                    <th>시비</th>
									<td><input type="text" name="CIT_AMT" id="CIT_AMT" value='<c:out value="${result.citAmt}"/>' class="input MX_11 DT_INT" />  원</td>				                    
								</tr>
								<tr>
									<th>기타비</th>
									<td colspan="3"><input type="text" name="ETC_AMT" id="ETC_AMT" value='<c:out value="${result.etcAmt}"/>' class="input MX_11 DT_INT" /> 원</td>			                    
								</tr>								
							</table>
						</div>
					</div>
