...
<%
  /**
  * @Class Name : 
  * @Description : RDL_TREE_PS 가로수 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","TRE_YMD", 10); // 사용개시일자
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>식재일자</th>
									<td><input type="text" name="TRE_YMD" id="TRE_YMD" value="<c:out value="${result.treYmd}"/>" class="input DT_DATE" />
								</tr>
								<tr>
									<th>행정읍/면/동</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>수종코드</th>
									<td><select name="TRE_CDE" id="TRE_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${tre_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.treCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>가로수직경</th>
									<td><input type="text" name="TRE_DIA" id="TRE_DIA" value='<c:out value="${result.treDia}"/>' class="input MX_5 DT_FLOAT DD_2" /> cm</td>		           
								</tr>				
								<tr>
									<th>수목보호판유무</th>
									<td colspan="3"><select name="PTC_CDE" id="PTC_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${ptc_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.ptcCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>					                         
								</tr>			
							</table>
