				...
				<div id="content2">
					...
					<div class="TabArea">
						...
						<div id="_Tab1" class="PSection on">
							<div class="Table_LBx">
								<div class="Table_left" style="height:175px">
									<table>
										<colgroup>
											<col width="19%" />
											<col width="31%" />
							...
							<div class="Table_LBx">
								<div class="Table_left">
									<table>
										...
										<tr>
										    <th>취수장명</th>
											<td><input type="text" name="GAI_NAM" id="GAI_NAM" value="<c:out value="${result.gaiNam}"/>" class="input MX_20 CS_20" /></td>
											<th>수원구분</th>
										...
										<tr>
											<th>평균취수량</th>
											<td><input type="text" name="AGA_VOL" id="AGA_VOL"  value="<c:out value="${result.agaVol}"/>" class="input MX_8 CS_8 DT_DOUBLE DD_0" /> ㎥/일</td>
											<th>최대취수량</th>
											<td><input type="text" name="HGA_VOL" id="HGA_VOL"  value="<c:out value="${result.hgaVol}"/>" class="input MX_8 CS_8 DT_FLOAT DD_0" /> ㎥/일</td>
										</tr>
										<tr>
											<th>펌프대수</th>
											<td><input type="text" name="PMP_CNT" id="PMP_CNT"  value="<c:out value="${result.pmpCnt}"/>" class="input MX_2 CS_2 DT_DOUBLE DD_0" /> </td>
											<th>부지면적</th>
											<td><input type="text" name="GAI_ARA" id="GAI_ARA"  value="<c:out value="${result.gaiAra}"/>" class="input MX_8 CS_8 DT_DOUBLE DD_2" /> ㎡</td>
										</tr>
										<tr>
											<th>취수방법</th>
											<td><select name="WGW_CDE" id="WGW_CDE"  class="select" >
													<option value=""></option>
													<c:forEach var="selectData" items="${wgw_cde_list}">
													<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.wgwCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
													</c:forEach>
												 </select>
											</td>
											<th>도수방법</th>
											<td><select name="WRW_CDE" id="WRW_CDE"  class="select" >
													<option value=""></option>
													<c:forEach var="selectData" items="${wrw_cde_list}">
													<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.wrwCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
													</c:forEach>
												 </select>
											</td>
										</tr>										
										<tr>
											<th>수계명</th>
											<td colspan="3"><input type="text" style="width:475px" name="WSS_NAM" id="WSS_NAM"  value="<c:out value="${result.wssNam}"/>" class="input MX_20 CS_20" /></td>											
										</tr>			
									</table>
								</div>
							</div>
