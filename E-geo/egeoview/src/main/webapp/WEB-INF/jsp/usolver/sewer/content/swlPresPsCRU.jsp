		...
		<div id="content2">
			...
			<div class="TabArea">
				...
				<div id="_Tab1" class="PSection on">
					<div class="Table_LBx">
						<div class="Table_left" style="height:130px">
							<table>
								<colgroup>
									<col width="19%" />
									<col width="31%" />
					...
					<div class="Table_LBx">
						<div class="Table_left" style="height:130px">
							<table>
								...
								<tr>
									<th>유수지명</th>
									<td colspan="3"><input type="text" name="PRS_NAM" id="PRS_NAM" value='<c:out value="${result.prsNam}"/>' class="input MX_20 CS_20" /></td>
								</tr>
								<tr>
									<th>유수지형태</th>
									<td><select name="SBS_CDE" id="SBS_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${sbs_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sbsCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
				                    <th>부지면적</th>
									<td><input type="text" name="PRS_ARA" id="PRS_ARA" value='<c:out value="${result.prsAra}"/>' class="input MX_8 CS_8 DT_FLOAT DD_2" /> ㎡</td>
								</tr>
								<tr>
									<th>저수용량</th>
									<td><input type="text" name="CTN_VOL" id="CTN_VOL" value='<c:out value="${result.ctnVol}"/>' class="input MX_8 CS_8 DT_INT" /> ㎥</td>
									<th>최저수위</th>
									<td><input type="text" name="LOW_WAL" id="LOW_WAL" value='<c:out value="${result.lowWal}"/>' class="input MX_5 CS_5 DT_FLOAT DD_2" /> m</td>									
								</tr>
								<tr>
									<th>계획홍수위</th>
									<td><input type="text" name="PRN_WAL" id="PRN_WAL" value='<c:out value="${result.prnWal}"/>' class="input MX_5 CS_5 DT_FLOAT DD_2" /> m</td>
									<th>최고수위</th>
									<td><input type="text" name="HGH_WAL" id="HGH_WAL" value='<c:out value="${result.hghWal}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>									
								</tr>								
							</table>
						</div>
					</div>
