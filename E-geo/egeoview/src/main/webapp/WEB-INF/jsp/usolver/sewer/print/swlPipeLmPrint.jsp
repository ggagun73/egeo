	...
	<div id="form_wrap">
        ...
        <table class="tbprint" summary="대 장">
			...
			<tbody>
                ...
                <tr>
					<th scope="row">용도</th>
					<td colspan="2"><c:out value="${result.sbaCdeNm}"/></td>
					<th scope="row">설치일자</th>
					<td colspan="2"><c:out value="${result.istYmd}"/></td>
				</tr>
				<tr>
					<th scope="row">규모</th>
					<td><c:out value="${result.litCdeNm}"/></td>
					<th scope="row">단면형태</th>
					<td><c:out value="${result.forCdeNm}"/></td>
					<th scope="row">재질</th>
					<td><c:out value="${result.mopCdeNm}"/></td>
				</tr>
				<tr>
					<th scope="row">관거길이</th>
					<td colspan="3">가로 : <c:out value="${result.pipHol}"/> m &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 세로 : <c:out value="${result.pipVel}"/> m</td>
					<th scope="row">구경</th>
					<td><c:out value="${result.pipDip}"/> mm</td>
				</tr>
				<tr>
					<th scope="row">심도</th>
					<td colspan="3">시점 : <c:out value="${result.begDep}"/> m &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 종점 : <c:out value="${result.endDep}"/> m</td>
					<th scope="row">연장</th>
					<td><c:out value="${result.pipLen}"/> m</td>
				</tr>
				<tr>
					<th scope="row">관저고</th>
					<td colspan="3">시점 : <c:out value="${result.sbkAlt}"/> m &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 종점 : <c:out value="${result.sblAlt}"/> m</td>
					<th scope="row">평균구배</th>
					<td><c:out value="${result.pipSlp}"/> %</td>
				</tr>
				<tr>
					<th scope="row">맨홀관리번호</th>
					<td colspan="3">시점 : <c:out value="${result.bomIdn}"/>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 종점 : <c:out value="${result.emoIdn}"/> </td>
					<th scope="row">차선통로수</th>
					<td><c:out value="${result.pipLin}"/></td>
				</tr>
				<tr>
					<th scope="row">우수배수면적</th>
					<td colspan="2"><c:out value="${result.pipSbo}"/> ha</td>
					<th scope="row">오수배수면적</th>
					<td colspan="2"><c:out value="${result.pipSbp}"/> ha</td>
				</tr>
				<tr>
					<th scope="row">유천시유속</th>
					<td colspan="2"><c:out value="${result.pipSbq}"/> m/sec</td>
					<th scope="row">청천시유속</th>
					<td colspan="2"><c:out value="${result.pipSbr}"/> m/sec</td>
				</tr>
				<tr>
				...
				<c:forEach begin="${fn:length(swtSutlHtList) > 0 ?  fn:length(swtSutlHtList): 0}"  end="${fn:length(swtSutlHtList) > 6 ?  fn:length(swtSutlHtList): 6}">
					<tr>
						<td class="lnone" scope="col" style="min-height:22px"></td>
	                    <td class="lnone" scope="col"></td>
	                    <td class="lnone" scope="col"></td>
