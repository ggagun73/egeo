...
<head>
	...
	$( document ).ready(function() {
		...
		if( sAction === "BATCHUP"){
			...
			BOOK.fn_get_gridAddInfo("SWT_ATTA_DT");			//세부시설현황
	...
	<div id="container2"> 
		<div id="content2">
					...
					<div class="Table_LBx">
						<div class="Table_left" style="height:200px">
							<table>
								...
								<tr>
									<th>하수펌프장명</th>
									<td><input type="text" name="PMP_NAM" id="PMP_NAM" value='<c:out value="${result.pmpNam}"/>' class="input MX_20" /></td>
				                    <th>일일처리용량</th>
									<td><input type="text" name="DAY_VOL" id="DAY_VOL" value='<c:out value="${result.dayVol}"/>' class="input MX_8 DT_INT" /> ㎥</td>				                    
								</tr>
								<tr>
									<th>개통상태</th>
									<td><select name="SOO_CDE" id="SOO_CDE" class="select">
										<option value=""></option>
				                    	<c:forEach var="selectData" items="${soo_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sooCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
				                    <th>최대저수용량</th>
									<td><input type="text" name="MAX_VOL" id="MAX_VOL"  value='<c:out value="${result.maxVol}"/>' class="input MX_8 DT_INT" /> ㎥</td>
								</tr>
								<tr>
									<th>펌프장용도</th>
									<td><select name="SBE_CDE" id="SBE_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${sbe_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sbeCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
									<th>청천시오수양수능력</th>
									<td><input type="text" name="COS_VOL" id="COS_VOL" value='<c:out value="${result.cosVol}"/>' class="input MX_8 DT_INT" /> ㎥/분</td>									
								</tr>
								<tr>
									<th>부지면적</th>
									<td><input type="text" name="PMP_ARA" id="PMP_ARA" value='<c:out value="${result.pmpAra}"/>' class="input MX_8 DT_FLOAT DD_2" /> ㎡</td>
									<th>우천시오수양수능력</th>
									<td><input type="text" name="UOS_VOL" id="UOS_VOL" value='<c:out value="${result.uosVol}"/>' class="input MX_8 DT_INT" /> ㎥/분</td>
								</tr>	
								<tr>
									<th>표고</th>
									<td><input type="text" name="PMP_ALT" id="PMP_ALT" value='<c:out value="${result.pmpAlt}"/>' class="input MX_7 DT_FLOAT DD_2" /> m</td>	
									<th>우수양수능력</th>
									<td><input type="text" name="USU_VOL" id="USU_VOL" value='<c:out value="${result.usuVol}"/>' class="input MX_8 DT_INT" /> ㎥/분</td>								
								</tr>		
								<tr>
									<th>수위</th>
									<td><input type="text" name="PMP_WAL" id="PMP_WAL" value='<c:out value="${result.pmpWal}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>	
									<th>방향각</th>
			...
			<div class="TabArea disBatchUp">
				<div class="TabBx">
					<ul class="Tabs">
						<li class="_TabSub1"><a href="#" class="Tab Tab_selected">세부시설현황</a></li>
					</ul>
				</div>
				<div id="_TabSub1" class="PSection on">					
					<div class="Btn_pd3">
						<div class="Btn"></div>
						<security:authorize ifAnyGranted ="ROLE_SEWER_BOOK_EDIT" >
						<div class="Btn"><a href="#" class="Btn_03" onclick="BOOK.fn_open_gridAddInfo('SWT_ATTA_DT');">추가</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_open_gridAddInfo('SWT_ATTA_DT','UPDATE');">수정</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_delete_gridAddInfo('SWT_ATTA_DT');">삭제</a></div>
						</security:authorize>
					</div>
					<div class="TableBx2">
						<div class="Table_list2">
							<table id="gridSwtAttaDt<c:out value='${nJDSKSubId}'/>"></table>
						</div>
					</div>
				</div>
			</div>
