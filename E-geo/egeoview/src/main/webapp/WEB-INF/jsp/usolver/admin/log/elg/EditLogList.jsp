...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$(".resultListHover").click(function(){
			...
			fn_egov_inqire_editLog(requstId);
		});	
	});
 	...
 	function fn_search_list(pageNo){
		...
		document.frm_manage.action = "<c:url value='/admin/log/elg/SelectEditLogList.do'/>";
		document.frm_manage.submit();

	} 
	
	function fn_egov_inqire_editLog(requstId){
		
		var url = "<c:url value='/admin/log/elg/SelectEditLog.do'/>?requstId="+requstId;
		window.parent.REGISTER.fn_open_nJDSKWindow("편집 로그정보조회",url,540,500,"registerOnHome");
	}
...
<div id="W_900">
    ...
    <div id="admin">
        ...
        <div class="admin_content">
        	...
        	<form name="frm_manage"  id="frm_manage" action ="<c:url value='/admin/log/elg/SelectEditLogList.do'/>" method="post">
            <div class="ContBx">
                <div class="schbx">
                    <div class="FR" >
                    	발생일자 :                        
                        ...
                        테이블명 :  
                        <input type="text" style="height:19px;width:80px" id="searchKeyword" name="searchKeyword" value="${searchVO.searchKeyword}"  class="input"  onKeyDown="press()"/>
                        FID :  
                        <input type="text" style="height:19px;width:60px" id="searchCnd" name="searchCnd" value="${searchVO.searchCnd}"  class="input"  onKeyDown="press()"/>
                ...
                <table class="tblist" summary="편집 로그">
				    ...
				    <tbody>
					    ...
					    <tr class="resultListHover">
                            ...
                            <td><c:out value="${resultList.logDate}"/></td>
                            <td><c:out value="${resultList.userName}"/></td>
                            <td><c:out value="${resultList.editType}"/></td>
	                        <td><c:out value="${resultList.g2TableName}"/></td>
                            <td><c:out value="${resultList.g2Id}"/></td>
                         </tr>
                        </c:forEach>
