        ...
        <table class="tbprint" summary="대 장">
			...
			<tbody>
              ...
              <tr>
				...
				<tr>
					<th colspan="2" scope="row">주차장명</th>
					<td colspan="3"><c:out value="${result.prkNam}"/></td>
					<th scope="row">설치일자</th>
					<td><c:out value="${result.opnYmd}"/></td>
				</tr> 			
				<tr>
					<th rowspan="3" scope="row">관리자</th>
					<th scope="row">주소</th>
					<td colspan="3"><c:out value="${result.mngAdr}"/></td>
					<th scope="row">주차장위치구분</th>
					<td><c:out value="${result.pstCde}"/></td>
				</tr> 			
				<tr>
					<th scope="row">전화번호</th>
					<td colspan="3"><c:out value="${result.prkTel}"/></td>
					<th scope="row">주차장요금구분</th>
					<td><c:out value="${result.chrCdeNm}"/></td>
				</tr> 
			    <tr>
					<th scope="row">주민/사업자등록번호</th>
					<td colspan="2"><c:out value="${result.idtNum}"/></td>
					<th scope="row">요금</th>
					<td colspan="2"><c:out value="${result.feeExp}"/></td>
				</tr> 		
				<tr>
					<th rowspan="3" scope="row">운영시간</th>
					<th scope="row">개장시간</th>
					<td colspan="2"><c:out value="${result.opnTim}"/></td>
					<th scope="row">폐장시간</th>
					<td colspan="2"><c:out value="${result.clsTim}"/></td>
				</tr> 		
				<tr>
					<th scope="row">주차면수</th>
					<td colspan="2"><c:out value="${result.prkCnt}"/>대</td>
					<th scope="row">전체면적</th>
					<td colspan="2"><c:out value="${result.prkAra}"/> ㎡</td>
				</tr> 		
				<tr>
					<th scope="row">장애인주차면수</th>
					<td colspan="2"><c:out value="${result.hkpCnt}"/>대</td>
					<th scope="row">주차방향</th>
					<td colspan="2"><c:out value="${result.pklCdeNm}"/></td>
				</tr> 			
				<tr>
				...
				<c:forEach begin="${fn:length(rdtPrsvDtList) > 0 ?  fn:length(rdtPrsvDtList): 0}"  end="${fn:length(rdtPrsvDtList) > 13 ?  fn:length(rdtPrsvDtList): 13}">			
					<tr>
