...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","STR_YMD", 10); // 사용개시일자
						...
						<div class="Table_left">
							<table>
								...
								<tr class='visInsert'>
									...
									<th>사용개시일자</th>
									<td><input type="text" name="STR_YMD" id="STR_YMD" value="<c:out value="${result.strYmd}"/>" class="input DT_DATE" />
								</tr>
								<tr>								
									<th>처리구역명</th>
									<td><input type="text" name="ADP_NAM" id="ADP_NAM" value='<c:out value="${result.adpNam}"/>' class="input MX_20" /></td>
									<th>고시번호</th>
									<td><input type="text" name="NOT_NUM" id="NOT_NUM" value='<c:out value="${result.notNum}"/>' class="input MX_20" /></td>
								</tr>
								<tr>
									<th>용도지역</th>
									<td><select name="USE_CDE" id="USE_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${use_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.useCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select>
									</td>
				                    <th>처리구역면적</th>
									<td><input type="text" name="ADP_SIZ" id="ADP_SIZ" value='<c:out value="${result.adpSiz}"/>' class="input MX_6 DT_INT" /> ㏊</td>				                    
								</tr>
								<tr>
									<th>처리구역인구수</th>
									<td><input type="text" name="ADP_POP" id="ADP_POP" value='<c:out value="${result.adpPop}"/>' class="input MX_8 DT_INT" /> 인</td>
				                    <th>계획처리인구수</th>
									<td><input type="text" name="PRN_POP" id="PRN_POP" value='<c:out value="${result.prnPop}"/>' class="input MX_8 DT_INT" /> 인</td>				                    
								</tr>
								<tr>	
									<th>오수발생량</th>
									<td><input type="text" name="SEW_VOL" id="SEW_VOL" value='<c:out value="${result.sewVol}"/>' class="input MX_8 DT_INT" /> ㎥/일</td>
									<th>대장초기화여부</th>
