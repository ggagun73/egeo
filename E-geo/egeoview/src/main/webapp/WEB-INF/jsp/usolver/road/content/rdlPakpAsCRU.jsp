...
<%
  /**
  * @Class Name : 
  * @Description : RDL_PAKP_AS 주차장 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","OPN_YMD", 10); // 사용개시일자
		...
		if( sAction === "BATCHUP"){
			...
			$('.activeWindow').find('.Table_left').attr('style','height:235px');
			BOOK.fn_edit_mode();
		...
		<div id="content2">
			...
			<div class="TabArea">
						...
						<div class="Table_left" style="overflow-x:hidden;height:235px">				
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="OPN_YMD" id="OPN_YMD" value="<c:out value="${result.opnYmd}"/>" class="input DT_DATE" /></td>			
								</tr>
								<tr>
									<th>행정읍/면/동</th>
									...
									<th>주차장위치구분</th>
									<td><select name="PST_CDE" id="PST_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${pst_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.pstCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>            										
								</tr>
								<tr>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left"  style="overflow-x:hidden;height:235px">				
							<table>
								...
								<tr>
									<th>주차방향</th>
									<td><select name="PKL_CDE" id="PKL_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${pkl_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.pklCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
				                     <th>주차요금구분</th>
									<td><select name="CHR_CDE" id="CHR_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${chr_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.chrCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>		         
								</tr>
								<tr>
									<th>주차면수</th>
									<td><input type="text" name="PRK_CNT" id="PRK_CNT" value='<c:out value="${result.prkCnt}"/>' class="input MX_4 DT_INT"/></td>		
				                    <th>요금</th>
									<td><input type="text" name="FEE_EXP" id="FEE_EXP" value='<c:out value="${result.feeExp}"/>' class="input MX_50 CS_50"/></td>		         
								</tr>	
								<tr>
									<th>전체면적</th>
									<td><input type="text" name="PRK_ARA" id="PRK_ARA" value='<c:out value="${result.prkAra}"/>' class="input MX_10 DT_FLOAT DD_2"/>  ㎡</td>		
				                    <th>장애자 주차면수</th>
									<td><input type="text" name="HKP_CNT" id="HKP_CNT" value='<c:out value="${result.hkpCnt}"/>' class="input MX_4 DT_INT "/> 대</td>		         
								</tr>	
								<tr>
									<th>운영시간-개장시간</th>
									<td><input type="text" name="OPN_TIM" id="OPN_TIM" value='<c:out value="${result.opnTim}"/>' class="input MX_5 CS_5"/> </td>		
				                    <th>운영시간-폐장시간</th>
									<td><input type="text" name="CLS_TIM" id="CLS_TIM" value='<c:out value="${result.clsTim}"/>' class="input MX_5 CS_5"/> </td>		         
								</tr>	
								<tr>
									<th>주차장명</th>
									<td colspan="3"><input type="text" name="PRK_NAM" id="PRK_NAM" value='<c:out value="${result.prkNam}"/>' style="width:400px"  class="input MX_30 CS_30"/></td>		         
								</tr>
								<tr>
									<th>관리자주소</th>
									<td colspan="3"><input type="text" name="MNG_ADR" id="MNG_ADR" value='<c:out value="${result.mngAdr}"/>'  style="width:400px" class="input MX_50 CS_50"/></td>		         
								</tr>	
								<tr>
									<th>관리자전화번호</th>
									<td><input type="text" name="PRK_TEL" id="PRK_TEL" value='<c:out value="${result.prkTel}"/>' class="input MX_30 CS_30"/></td>	
									<th>주민/사업자등록번호</th>
									<td><input type="text" name="IDT_NUM" id="IDT_NUM" value='<c:out value="${result.idtNum}"/>' class="input MX_14 CS_14"/></td>		         	         
								</tr>									
							</table>
						</div>
					</div>
				...
				<div id="_Tab3" class="PSection">
					<div class="Table_LBx"   style="overflow-x:hidden;height:235px">				
						<div class="Btn_pd3"> 
							<security:authorize ifAnyGranted ="ROLE_ROAD_BOOK_EDIT" >
			...
			<div class="Btn_R">
				<%-- <div class="Btn disBatchUp"><a href="#" class="Btn_02"  onclick="BOOK.fn_view_image('<c:out value="${result.ftrCde}"/>','<c:out value="${result.ftrIdn}"/>','','RDT_IMGE_ET');">도면/사진</a></div> --%>
