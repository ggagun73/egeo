							...
							<div class="Table_LBx">
								<div class="Table_left" style="height:140px">
									<table>
										...
										<tr>
											<th>유량계종류</th>
											<td><select name="GAG_CDE" id="GAG_CDE"  class="select" >
													<option value=""></option>
													<c:forEach var="selectData" items="${gag_cde_list}">
													<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.gagCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
													</c:forEach>
												</select></td>
											<th>유량계형식</th>
										...
										<tr>
											<th>구경</th>
											<td><input type="text" name="FLO_DIP" id="FLO_DIP" value='<c:out value="${result.floDip}"/>' class="input MX_4 CS_4 DT_INT DD_0" /> mm</td>
											<th>제작회사명</th>
