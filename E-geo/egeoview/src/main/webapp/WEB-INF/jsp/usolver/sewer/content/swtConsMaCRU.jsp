...
<%
  /**
  * @Class Name : 
  * @Description :   SWT_CONS_MA   하수공사대장
...
<head>
	...
	<script type="text/javascript">
		...
		$( document ).ready(function() {
			...
			// 달력
			BOOK.fn_create_datepicker("frm_cons", "BID_YMD" , 10);
			BOOK.fn_create_datepicker("frm_cons", "CTT_YMD" , 10);
			BOOK.fn_create_datepicker("frm_cons", "BEG_YMD" , 10);
			BOOK.fn_create_datepicker("frm_cons", "FNS_YMD" , 10);
			BOOK.fn_create_datepicker("frm_cons", "RFN_YMD" , 10);
			BOOK.fn_create_datepicker("frm_cons", "FCH_YMD" , 10);
			...
			if( sAction === "BATCHUP" || sAction === "INSERT"){
				...
				BOOK.fn_get_gridAddInfo("SWT_COST_DT");	// 공사비지급내역
				BOOK.fn_get_gridAddInfo("SWT_CHNG_DT");	// 설계변경내역
				BOOK.fn_get_gridAddInfo("SWT_SUBC_DT");	// 하도급내역
				BOOK.fn_get_gridAddInfo("SWT_FLAW_DT");	// 하자보수내역
			}
...
<input type="hidden" id="openerId" name="openerId" value='<c:out value="${openerId}"/>' /><!-- nJDSK window창(검색목록)의 Sub창들 관리를 위해-->
...
<input type="hidden" id="key_colunm" name="key_colunm"  value="CNT_NUM"/>
		...
		<div id="content2">
			<div class="TxtBg">
				<dl class="disBatchUp">
					<dl>
						...
						<dt>공사명</dt>
						<dd><c:out value="${result.cntNam}" /></dd>
					</dl>
				</dl>
			</div>
			<!-- tab -->
			<div class="TabArea">
				...
				<div id="_Tab1" class="PSection on">
					<div class="Table_LBx">
						<div class="Table_left" style="height:167px">
							<table>
								<colgroup>
									<col width="19%" />
									<col width="31%" />
								...
								<tr>
									<th>공사구분</th>
									<td><select name="CNT_CDE" id="CNT_CDE" class="select">
											<option value=""></option>
											<c:forEach var="selectData" items="${cnt_cde_list}">
												<option value="${selectData.CODE}"
													<c:if test = "${selectData.CODE == result.cntCde}"> selected="selected" </c:if>>${selectData.VAL}</option>
											</c:forEach>
									</select></td>
									<th>설계자명</th>
								...
								<tr>
									...
									<th>관급금액</th>
									<td><input type="text" name="DGV_AMT" id="DGV_AMT"  value='<c:out value="${result.dgvAmt}"/>'class="input MX_11 CS_11 DT_DOUBLE DD_0" /></td>
								</tr>
								<tr>
									<th>순공사비</th>
						...
						<div class="Table_left"  style="height:167px">
							<table>
								...
								<tr>
									...
									<input type="hidden" id="DATE_FCH" value="<c:out value="${result.fchYmd}"/>"/></td>
									<th>실준공일자</th>
								...
								<tr>
									<th>관급물량</th>
									<td colspan="3"><input type="text"  style="width: 475px;" name="GVR_DES"  id="GVR_DES"  value='<c:out value="${result.gvrDes}"/>'  class="input MX_500 CS_500" /></td>
								</tr>
							</table>
						</div>
					</div>
					...
					<div class="Table_LBx">
						<div class="Table_left"  style="height:167px">
							<table>
								...
								<tr>
									<th>세항</th>
									<td><input type="text"  name="SHN_EXP" id="SHN_EXP" value='<c:out value="${result.shnExp}"/>' class="input MX_30 CS_30" /></td>
									<th>목</th>
			...
			<div class="TabArea disBatchUp">
				...
				<div id="_TabSub1" class="PSection on">
					<div class="Btn_pd3">
						<security:authorize ifAnyGranted ="ROLE_SEWER_BOOK_EDIT" >
						<div class="Btn"><a href="#" class="Btn_03" onclick="BOOK.fn_open_gridAddInfo('SWT_COST_DT','');">추가</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_open_gridAddInfo('SWT_COST_DT','UPDATE');">수정</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_delete_gridAddInfo('SWT_COST_DT');">삭제</a></div>
						</security:authorize>
					</div>
					<div class="TableBx2">
 						<div class="Table_list2">
							<table id="gridSwtCostDt<c:out value='${nJDSKSubId}'/>"></table>
						</div>
					</div>
				</div>
				<div id="_TabSub2" class="PSection">
					<div class="Btn_pd3">
						<security:authorize ifAnyGranted ="ROLE_SEWER_BOOK_EDIT" >
						<div class="Btn"><a href="#" class="Btn_03" onclick="BOOK.fn_open_gridAddInfo('SWT_CHNG_DT','');">추가</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_open_gridAddInfo('SWT_CHNG_DT','UPDATE');">수정</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_delete_gridAddInfo('SWT_CHNG_DT');">삭제</a></div>
						</security:authorize>
					</div>
					<div class="TableBx2">
						<div class="Table_list2">
							<table id="gridSwtChngDt<c:out value='${nJDSKSubId}'/>"></table>
						</div>
					</div>
				</div>
				<!--  PSection End -->
				<div id="_TabSub3" class="PSection">				
					<div class="Btn_pd3">
						<security:authorize ifAnyGranted ="ROLE_SEWER_BOOK_EDIT" >
						<div class="Btn"><a href="#" class="Btn_03" onclick="BOOK.fn_open_gridAddInfo('SWT_SUBC_DT','');">추가</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_open_gridAddInfo('SWT_SUBC_DT','UPDATE');">수정</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_delete_gridAddInfo('SWT_SUBC_DT');">삭제</a></div>
						</security:authorize>
					</div>				
					<div class="TableBx2">
						<div class="Table_list2">
							<table id="gridSwtSubcDt<c:out value='${nJDSKSubId}'/>"></table>
						</div>
					</div>
				</div>
				<!--  PSection End -->
				<div id="_TabSub4" class="PSection">
					<div class="Btn_pd3">
						<security:authorize ifAnyGranted ="ROLE_SEWER_BOOK_EDIT" >
						<div class="Btn"><a href="#" class="Btn_03" onclick="BOOK.fn_open_gridAddInfo('SWT_FLAW_DT','');">추가</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_open_gridAddInfo('SWT_FLAW_DT','UPDATE');">수정</a></div>
						<div class="Btn"><a href="#" class="Btn_03"onclick="BOOK.fn_delete_gridAddInfo('SWT_FLAW_DT');">삭제</a></div>
						</security:authorize>
					</div>
					<div class="TableBx2">
						<div class="Table_list2">
							<table id="gridSwtFlawDt<c:out value='${nJDSKSubId}'/>"></table>
						</div>
					</div>
				</div>
				<!--  PSection End -->
			...
			<div class="Btn_R">
				<div class="Btn disBatchUp"><a href="#" class="Btn_02"  onclick="BOOK.fn_view_image('','','<c:out value="${result.cntNum}"/>','SWT_IMGE_ET');">도면/사진</a></div>
				<security:authorize ifAnyGranted ="ROLE_SEWER_BOOK_PRINT" >
				<div class="Btn disBatchUp"><a href="#" class="Btn_02"  onclick="BOOK.fn_print_report('SWT_CONS_MA','<c:out value="${result.g2Id}"/>');">출력</a></div>
				</security:authorize>
