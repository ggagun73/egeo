...
<%
  /**
  * @Class Name : 
  * @Description : RDL_TFSN_PS 교통표지판 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","IPG_YMD", 10); // 사용개시일자
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="IPG_YMD" id="IPG_YMD" value="<c:out value="${result.ipgYmd}"/>" class="input DT_DATE" /></td>			
								</tr>
								<tr>
									<th>행정읍/면/동</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>표지판구분</th>
									<td><select name="AET_CDE" id="AET_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${aet_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.aetCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
				                    <th>높이</th>
									<td><input type="text" name="DPG_HIT" id="DPG_HIT" value='<c:out value="${result.dpgHit}"/>' class="input MX_2 DT_INT" /> m</td>		         
								</tr>
								<tr>
									<th>지주형식</th>
									<td><select name="IPG_CDE" id="IPG_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${ipg_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.ipgCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>규격</th>
									<td><input type="text" name="DPG_EXP" id="DPG_EXP" value='<c:out value="${result.dpgExp}"/>' class="input MX_50 CS_50" /> </td>		         
								</tr>	
								<tr>
									<th>위치구분</th>
									<td colspan="3"><select name="PLC_CDE" id="PLC_CDE"  class="select" >
											<option value=""></option>
								...
								<tr>
									<th>기재사항</th>
									<td colspan="3"><input type="text" name="IPG_DES" id="IPG_DES" value='<c:out value="${result.ipgDes}"/>'  style="width:400px" class="input MX_100 CS_100" /></td>		
								</tr>							
							</table>
						</div>
					</div>
