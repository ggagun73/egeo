...
<%
  /**
  * @Class Name : 
  * @Description : RDL_PROT_LS 방호울타리 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","WOL_YMD", 10); // 사용개시일자
							...
							<table>
								...
								<tr>
									...
									<td><select name="FTR_CDE" id="FTR_CDE"  class="select" >
											...
											<c:if test="${selectData.CODE eq 'AE220' ||  selectData.CODE eq 'AE221' || selectData.CODE eq 'AE222' }">
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="WOL_YMD" id="WOL_YMD" value="<c:out value="${result.wolYmd}"/>" class="input DT_DATE" />
								</tr>
								<tr>
									<th>공사번호</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>방지책종류</th>
									<td><select name="AED_CDE" id="AED_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${aed_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.aedCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
				                    <th>높이</th>
									<td><input type="text" name="WOL_HIT" id="WOL_HIT" value='<c:out value="${result.wolHit}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		          
								</tr>
								<tr>
									<th>재질</th>
									<td><select name="SMG_CDE" id="SMG_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${smg_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.smgCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>연장</th>
									<td><input type="text" name="WOL_LEN" id="WOL_LEN" value='<c:out value="${result.wolLen}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		         
								</tr>							
							</table>
						</div>
					</div>
