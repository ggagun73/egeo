...
<%
  /**
  * @Class Name : 
  * @Description :   WTL_STPI_PS   스탠드파이프관리
								...
								<div class="Table_left" style="height:120px">
									<table>
										...
										<tr>
											...
											<th>재질</th>
											<td><select name="STP_MOP" id="STP_MOP"  class="select" >
													<option value=""></option>
													<c:forEach var="selectData" items="${stp_mop_list}">
													<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.stpMop}"> selected="selected" </c:if> >${selectData.VAL}</option>
													</c:forEach>
												</select></td>
										</tr>
										<tr>
											<th>구경</th>
											<td><input type="text" name="STP_DIP" id="STP_DIP" value='<c:out value="${result.stpDip}"/>' class="input MX_4 CS_4 DT_INT DD_0" /> mm</td>
											<th>제작회사명</th>
											<td><input type="text" name="PRD_NAM" id="PRD_NAM" value='<c:out value="${result.prdNam}"/>' class="input MX_20 CS_20" /></td>
										</tr>
										<tr>
											<th>표고</th>
											<td><input type="text" name="STP_ALT" id="STP_ALT" value='<c:out value="${result.stp_alt}"/>' class="input MX_7 CS_7 DT_FLOAT DD_2" /> m</td>
											<th>방향각</th>
