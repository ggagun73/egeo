		...
		<div id="content2">
			...
			<div class="TabArea">
				...
				<div id="_Tab1" class="PSection on">
					<div class="Table_LBx">
						<div class="Table_left" style="height:250px">
							<table>
								<colgroup>
									<col width="19%" />
									<col width="31%" />
					...
					<div class="Table_LBx">
						<div class="Table_left" style="height:250px">
							<table>
								...
								<tr>
									<th>하수처리장명</th>
									<td><input type="text" name="DRN_NAM" id="DRN_NAM" value='<c:out value="${result.drnNam}"/>' class="input MX_20" /></td>
				                    <th>방류수역명</th>
									<td><input type="text" name="DRA_NAM" id="DRA_NAM" value='<c:out value="${result.draNam}"/>' class="input MX_20" /> </td>				                    
								</tr>
								<tr>
									<th>개통상태</th>
				                    ...
				                    <th>청천시 처리용량</th>
									<td><input type="text" name="PCC_VOL" id="PCC_VOL"  value='<c:out value="${result.pccVol}"/>' class="input MX_8 DT_INT" /> ㎥/일</td>
								</tr>
								<tr>
									<th>하수처리방식</th>
									<td><select name="SBB_CDE" id="SBB_CDE" class="select">
		                            <option value=""></option>
				                    	<c:forEach var="selectData" items="${sbb_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sbbCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
										</c:forEach>
				                    </select></td>
									<th>우천시 처리용량</th>
									<td><input type="text" name="PUC_VOL" id="PUC_VOL" value='<c:out value="${result.pucVol}"/>' class="input MX_8 DT_INT" /> ㎥/일</td>									
								</tr>
								<tr>
									<th>부지면적</th>
									<td><input type="text" name="DRN_ARA" id="DRN_ARA" value='<c:out value="${result.drnAara}"/>' class="input MX_8 DT_FLOAT DD_2"" /> ㎡</td>
									<th>처리구역면적</th>
									<td><input type="text" name="ADP_SIZ" id="ADP_SIZ" value='<c:out value="${result.adpSiz}"/>' class="input MX_8 DT_FLOAT DD_2" /> ㎡</td>
								</tr>	
								<tr>
									<th>차집관 연장</th>
									<td><input type="text" name="PIP_LEN" id="PIP_LEN" value='<c:out value="${result.pipLen}"/>' class="input MX_7 DT_FLOAT DD_2" /> m</td>	
									<th>방향각</th>
									<td><input type="text" name="ANG_DIR" id="ANG_DIR" value='<c:out value="${result.angDir}"/>' class="input MX_3 DT_INT" /> </td>								
								</tr>		
								<tr>
									<th>설계유입수 수질</th>
									<td colspan="3"><input type="text"  style="width:375px" name="DRN_SB1" id="DRN_SB1" value='<c:out value="${result.drnSb1}"/>' class="input MX_50" /> </td>
								</tr>
								<tr>
									<th>설계유출수 수질</th>
									<td colspan="3"><input type="text"  style="width:375px" name="DRN_SB2" id="DRN_SB2" value='<c:out value="${result.drnSb2}"/>' class="input MX_50" /></td>
								</tr>	
							</table>
						</div>
					</div>
