...
<%
  /**
  * @Class Name : 
  * @Description : RDL_PDCR_AS 횡단보도 관리 
...
<head>
	...
	$( document ).ready(function() {
		...
		// 1 )달력
		BOOK.fn_create_datepicker("frm_detail","CRS_YMD", 10); // 사용개시일자
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									...
									<th>설치일자</th>
									<td><input type="text" name="CRS_YMD" id="CRS_YMD" value="<c:out value="${result.crsYmd}"/>" class="input DT_DATE" /></td>			
								</tr>
								<tr>
									<th>행정읍/면/동</th>
					...
					<div class="Table_LBx">
						...
						<div class="Table_left" style="height:169px">	
							<table>
								...
								<tr>
									<th>신호등유무</th>
									<td><select name="SGB_CDE" id="SGB_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${sgb_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.sgbCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>	
				                     <th>횡단보도폭</th>
									<td><input type="text" name="CRS_WID" id="CRS_WID" value='<c:out value="${result.crsWid}"/>' class="input MX_5 DT_FLOAT DD_2" /> m</td>		         
								</tr>
								<tr>
									<th>장애인시설유무</th>
									<td><select name="HND_CDE" id="HND_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${hnd_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.hndCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>횡단보도길이</th>
									<td><input type="text" name="CRS_LEN" id="CRS_LEN" value='<c:out value="${result.crsLen}"/>' class="input MX_5 DT_FLOAT DD_2"/> m</td>		         
								</tr>	
								<tr>
									<th>보도턱낮춤유무</th>
									<td><select name="BDN_CDE" id="BDN_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${bdn_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.bdnCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>
				                    <th>횡단보도방식</th>
									<td><select name="CRW_CDE" id="CRW_CDE"  class="select" >
											<option value=""></option>
											<c:forEach var="selectData" items="${crw_cde_list}">
											<option value="${selectData.CODE}" <c:if test = "${selectData.CODE == result.crwCde}"> selected="selected" </c:if> >${selectData.VAL}</option>
											</c:forEach>
										</select></td>         
								</tr>													
							</table>
