    	...
    	if(NUTs.Util.isEmptyObject(oEditRuleInfo.operType.modify) === false){
    		...
    		if(oEditRuleInfo.operType.modify.length > 0){	// 'checkRelationGeometryMove' or 'checkRelationGeometryMoveEndPoint' 일때
    			...
    			for(var i=0,len=oEditRuleInfo.operType.modify.length; i<len; i++){
    				...
    				if(NUTs.Util.isEmptyObject(EditRule.resultGeometry) === false){
...
NUTs.Format.GML = OpenLayers.Class(OpenLayers.Format.GML, {
    ...
    parseGeometry: {
        ...
        //CJH 2017.03 for geoserver
        getSrsDimension: function(node){
        	var dim = 2; 
        	var maxLoop = 10;
        	var curLoop = 0;
        	while(node && node.parentNode){
        		...
        		if(curLoop <= maxLoop){
            		...
            		if(node.getAttribute("srsDimension")){
            			dim = node.getAttribute("srsDimension");
            			break;
            		}
        ...
        linestring: function(node, ring) {
            ...
            if(nodeList.length > 0) {
                ...
                if(!dim && node){ //CJH 2017-03-23 for geoserver
                	 
                	dim = this.parseGeometry.getSrsDimension(node);
    ...
    writers: OpenLayers.Util.applyDefaults({
        "sld": {
            ...
            "MinScaleDenominator": function(scale) {
            	if(!scale || String(scale).toUpperCase() == "NAN")
            		scale = 0;
   ...
   /**
    ...
    writers: OpenLayers.Util.applyDefaults({
        "sld": OpenLayers.Util.applyDefaults({
            ...
            "PointSymbolizer": function(symbolizer) {
                var writers = NUTs.Format.SLD.v1_0_0.prototype.writers;
...
NUTs.WFS = {
	...
	SRSNAME : 'srsName="EPSG:5181"',
	
	REQUEST : null,
	
	format : {
		gml : new NUTs.Format.GML(),
		//gml2 : new NUTs.Format.GML.v2(),
		//gml3 : new NUTs.Format.GML.v3(),
...
//			queryStr += this.format.filter.write(filter).xml;
	...
	update : function(serviceUrl, features, prefix, table, fields, values, value, callback) {
		...
		wfsStr += '<ogc:PropertyName>FID</ogc:PropertyName> '; //CJH 2017.03.29 for geoserver : tablename.FID => FID
			...
			if(this.mode){
				  ...
				  function addDataToViewLayer(_oShp){
					  for(var idx = 0; idx < importedFeatureLen ; idx++) {
						  ...
						  if(shpType.toLowerCase() == "linestring"){
							  featureSumInfo += oGInnerFeature.geometry.getLength();
							  featureInfoNam = "연장";
							  featureInfoUnit = "m";
						  }
						  else if(shpType.toLowerCase() == "polygon"){
...
//			if ((this.map.layers[i].CLASS_NAME == "GWMS" || this.map.layers[i].CLASS_NAME == "GWMSPost") && this.map.layers[i].visibility) {
			if ((this.map.layers[i].CLASS_NAME == "NUTs.Layer.WMS") && this.map.layers[i].visibility) {
				...
				if(layerTool){
					var layerList = layerTool.getLayerShowList().join(",")
