...
GMap = OpenLayers.Class(OpenLayers.Map, {
            			...
            			this.controls.push(new GMeasure(GPathMeasure, {
							...
							eventListeners : {
								"measure" : function(evt) {
									...
									$(gMap.div).css("cursor", "move");
								}
							}
						}));
            		}
            		if(NUTs.Handler.PolygonMeasure){
            			this.controls.push(new GMeasure(NUTs.Handler.PolygonMeasure, {
							id : "measurePolygon"
            			}));
            		}
            	}*/
	...
	cleanMap : function(_aExceptionLayers) {
		...
		for(var i in this.layers) {
			if(this.layers[i].CLASS_NAME == "GVector" || this.layers[i].CLASS_NAME == "OpenLayers.Layer.Vector") {
				if(_aExceptionLayers){
					if(_aExceptionLayers.indexOf(layers[i]) === -1)
						this.layers[i].removeFeatures(this.layers[i].features);
				}
				else
					this.layers[i].removeFeatures(this.layers[i].features);
