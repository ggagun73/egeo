...
GRequest.WMS = {
    ...
    getStyles: function(serviceUrl, layers, callback) {
        var params = {
			...
			//version : this.version,
			version : "1.1.1",
			//add jykw 20160708 for geoserver
            request: "GetStyles",		
            layers: layers
        };
        var obj = this;
        ...
        GMapUtil.sendProxyPost(serviceUrl, GUtil.fn_convert_objToStr(params).slice(0,-1), function(res) {
            obj.parseGetStyles(res, callback)
        })
    },	
        ...
        for (var i in namedLayers) {
            ...
            for (var j = 0, jLen = userStyles.length; j <
                ...
                for(var k = 0, kLen = rules.length; k < kLen; k++) {
					...
					var minScaleVal = rules[k].minScaleDenominator;
					// ggash 20170111 for geoserver - GeoServer의 경우 layerFeatureConstraints property가 없음
					if(!rules[k].hasOwnProperty('minScaleDenominator')){
...
GWMS = OpenLayers.Class(OpenLayers.Layer.WMS, {
    ...
    yx : {'EPSG:5181' : true}, //ggash 20170112 for geoserver
...
GDrawFeature = OpenLayers.Class(OpenLayers.Control.DrawFeature, {
    ...
    drawFeature: function(geometry, attributes) {
        if (attributes && attributes.featureType && attributes.featureType == "Text") this.removeInputTextPopup();
        ...
        if (attributes && attributes.featureType && attributes.featureType == "Text") this.addInputTextPopup(feature)
    },
    ...
    removeInputTextPopup: function() {
        ...
        for (var i = len - 1; i >= 0; i--)
            if (this.layer.features[i].attributes.featureType == "Text") this.layer.removeFeatures(this.layer.features[i])
    },
    addTextPopup: function() {
        ...
        $(".olControlDrawText").click(this.map, function() {
            ...
            if (map.getControl("drawSelect") && map.getControl("drawSelect").active) map.getControl("drawSelect").selectTextPopup(this);
            else if (map.getControl("drawEdit") && map.getControl("drawEdit").active) map.getControl("drawEdit").selectTextPopup(this)
        });
...
GModifyFeature = OpenLayers.Class(OpenLayers.Control.ModifyFeature, {
    ...
    initialize: function(layer, options) {
        ...
        if (this.standalone === false) this.selectControl =
            new GSelectFeature(layer, selectOptions);
        var dragCallbacks = {
            down: function(pixel) {
                this.vertex = null;
...
/*        if (!this.alias) this.getRequestAlias(resObj, success);
        else this.events.triggerEvent(this.EVENT_TYPES[0], {
            data: resObj,
            success: function() {
                return success
            }
...
GDrawTool = OpenLayers.Class({
    ...
    initialize: function(map, options) {
        ...
        if (options) {
            ...
            if (options.onModificationStart) map.getControl("drawEdit").onModificationStart = options.onModificationStart;
            if (options.onSelect) map.getControl("drawSelect").onSelect =
                options.onSelect;
            if (options.onDeactivate) map.getControl("drawEdit").onDeactivate = options.onDeactivate;
            ...
            if (this.map.layers[i].CLASS_NAME == "GVector" && this.map.layers[i].visibility) {
                ...
                for (var j = 0; j < this.map.layers[i].features.length; j++)
                    if (this.map.layers[i].features[j].geometry.CLASS_NAME == "OpenLayers.Geometry.Point") {
                        //if (!this.map.layers[i].features[j].attributes.featureType) continue;
                        var params;
                        ...
                        if (/*this.map.layers[i].features[j].attributes.featureType ==
                            "Image"*/style.externalGraphic) params = {
    ...
    getSld_body: function(ruleView, baseLayer) {
        ...
        if (ruleView) {
            ...
            for (var i in sld.namedLayers) {
                var name = "";
                //ggash 2016.12.28 for geoserver
