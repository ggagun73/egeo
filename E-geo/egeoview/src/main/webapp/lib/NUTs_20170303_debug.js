    ...
    onComplete: function(feature, pixel) {
        ...
        if(COMMON.isEmptyObject(oEditRuleInfo.operType.modify) === false){
        	...
        	if(oEditRuleInfo.operType.modify.length > 0){	// 'checkRelationGeometryMove' or 'checkRelationGeometryMoveEndPoint' 일때
        		...
        		EditRule.editingGeometry = olFeature;
        		...
        		if(olFeature.geometry.CLASS_NAME.replace('OpenLayers.Geometry.','') === 'LineString'){
        			...
        			EditRule.offset.x = olFeature.geometry.components[0].x - olFeature.modified.geometry.components[0].x;
        			EditRule.offset.y = olFeature.geometry.components[0].y - olFeature.modified.geometry.components[0].y;
        		}
    ...
    drawFeature: function (geometry) {
    		...
    		if(DrawPath.crossOption){
    			switch(DrawPath.crossOption){
	    			...
	    			case 'cut':				// [접합] 입력 feature와 교차하는 개체를 찾아서 발견되면, 입력한 feature와의 교차점에서 분할처리(교차점부터 입력 feature의 끝점까지는 제거함)
	    				var oIntersectsGeom = EditRule.checkRelationGeometry(feature, sEditLayer, EditRule.spatialOperType.INTERSECTS);
	    					...
	    					if(oIntersectsGeom.data.length === 1){
	    						if(oIntersectsGeom.data[0].results.length >= 2)
	    							...
	    							if(nSCompDist > nECompDist){
	    								...
	    								if(nSCompDist < EditRule.tolerance){		// 접점(ex. 스냅을 걸었을 경우)에 해당하는 경우
	    									;	// do nothing
	    								}
	    								else{
	    									var aDividedObj = editor.getDivideLine(feature.geometry.components, oIntersectsGeom.data[0].results[0].feature.geometry.components);
        ...
        if (proceed !== false && geometry.getLength() >= this.minLength) {   
            ...
            if(COMMON.isEmptyObject(oEditRuleInfo.operType.add) === false){
            	var sErrorMsg = [];
        		EditRule.editingGeometry = feature;
            	...
            	for(var i=0,len=oEditRuleInfo.operType.add.length; i<len; i++){
            		...
            		if(sEditRule.indexOf('checkRelationGeometry') !== -1){
            			if(COMMON.isEmptyObject(EditRule.resultGeometry) === false)
            			...
            			if(COMMON.isEmptyObject(EditRule.resultGeometry) === false){
            				oResultLastPoint = EditRule.resultGeometry;
	 ...
	 title: OpenLayers.i18n('oleDrawPoint'),
	    ...
	    drawFeature: function (geometry) {
	    	...
	    	if (proceed !== false) {
	            ...
	            var oEditRuleInfo = MAP_EDITOR.fn_get_editRuleInfo();	            
	            if(COMMON.isEmptyObject(oEditRuleInfo.operType.add) === false){		// 룰 적용 ex) EditRule.checkRelationGeometryEnd, checkRelationGeometry(Contains)
	            	var sErrorMsg = [];
	            	EditRule.editingGeometry = feature;
	            	...
	            	for(var i=0,len=oEditRuleInfo.operType.add.length; i<len; i++){
	            		...
	            		if(COMMON.isEmptyObject(EditRule.resultGeometry) === false){	            				            			
	            			if(EditRule.resultGeometry.data.length > 0){
	            				
	            				// WTL_VALV_PS 이면 교차된 상수관 분할처리
	            				if(sEditLayer === 'WTL_VALV_PS'){
	            					editor.addUnDrawFeature(editor.refLayer, oGFeature.feature);
	            					editor.divideLine(oGFeature, EditRule.resultGeometry.data[0].results[0].feature);
...
NUTs.Edit.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control.ModifyFeature, {
    ...
    dragVertex: function(vertex, pixel) {
        ...
        this.modified = true;        
        EditRule.dist = COMMON.fn_get_DistanceBy2Point(geomClone.x, geomClone.y, geom.x, geom.y);        // vertex 정점편집 시, 이동거리 - 2016.7.28 ehyun
	...
	dragComplete: function(vertex) {
    	...
    	if(COMMON.isEmptyObject(oEditRuleInfo.operType.modify) === false){
    		...
    		if(oEditRuleInfo.operType.modify.length > 0){	// 'checkRelationGeometryMove' or 'checkRelationGeometryMoveEndPoint' 일때
    			...
    			EditRule.editingGeometry = this.feature;
        ...
        this.events.on({
            'transformcomplete': function(e){
            	...
            	this.drawTransformFeature = function(){            		
                	...
                	EditRule.CheckRelationLocValueSync(oOriginGInnerFeatureClone);
                ...
                if(COMMON.isEmptyObject(oEditRuleInfo.operType.modify) === false){
                	...
                	if(oEditRuleInfo.operType.modify.length > 0){	// 'checkRelationGeometryMove' or 'checkRelationGeometryMoveEndPoint' 일때
                		...
                		EditRule.editingGeometry = e.feature;
                		...
                		if(e.feature.geometry.CLASS_NAME.replace('OpenLayers.Geometry.','') === 'LineString'){
                			...
                			EditRule.offset.x = e.feature.geometry.components[0].x - e.feature.modified.geometry.components[0].x;
                			EditRule.offset.y = e.feature.geometry.components[0].y - e.feature.modified.geometry.components[0].y;
                		}
    ...
    addPoint: function(pixel) {
        ...
        if(DrawPath.mode && DrawPath.crossOption !== 'cut'){
        	...
        	if(DrawPath.forDivideFeature && DrawPath.forDivideFeature.geometry.components.length > 1){			
        		...
        		var oIntersectsGeom = EditRule.checkRelationGeometry(oTmpPointFeatureForCalc, sEditLayer, EditRule.spatialOperType.INTERSECTS);
        		if(COMMON.isEmptyObject(oIntersectsGeom) === false && oIntersectsGeom.data.length === 1){
        			if(oIntersectsGeom.data[0].results.length >= 2){
        				...
        				if((nSCompDist > nECompDist) || (nSCompDist < nECompDist && nSCompDist > EditRule.tolerance)){
        					switch(DrawPath.crossOption){
        					case 'cross' : 
        									// #0. 입력선분할
...
NUTs.WMS = {
	...
	getStyles : function(serviceUrl, layers, callback) {
		...
		NUTs.Util.sendProxyPost(serviceUrl, NUTs.Util.fn_convert_objToStr(params).slice(0,-1), function(res) {
            obj.parseGetStyles(res, callback)
        })
	},
		...
		addDataOnMap : function(arrObj){
			...
			if(this.mode){
				  ...
				  function addDataToViewLayer(_oShp){
					  ...
					  COMMON.showMessage('외부파일(shp) 로딩결과 & * 파일명 : [ <strong>' + sSelectedDataFile + '.shp</strong> ]<br/>* 피쳐 개수  : [ <strong>' + importedFeatureLen + '</strong> ]개 <br/>* 총 ' + featureInfoNam + ' : [ <strong>' + NUTs.Util.fn_fmt_cur(featureSumInfo.toFixed(2)) + '</strong> ]' + featureInfoUnit + '<br/>* 데이터 유효성 점검 결과 : <span style="color:blue">이상 없음</span>', 10000);
					  return false;
...
NUTs.Tool.TMapLayerTool = OpenLayers.Class({
	...
	initialize : function(layers, tMaps, groups, layerGroups, options) {
		...
		if(options && options.serviceUrl && options.callback) {
			...
			if(options.sync){
				NUTs.WMS.getStylesBySync(options.serviceUrl,this.getLayers({retAttr:"theme"})+',', function(res) {
				...
				NUTs.WMS.getStyles(options.serviceUrl,this.getLayers({retAttr:"theme"})+',', function(res) {
					control.defaultSld = res.xml.cloneNode(true);
					var bool = false;
					if(options.userStyle) {
						control.setUserStyle(options.userStyle, res);
	...
	setUserStyle : function(userStyle, res) {
		...
		for(var i=0, len=userNamedLayers.length; i < len; i++) {
			...
			if(element.length > 0) userName = CONFIG.fn_get_dataHouseName + ":" + element[0].text;
