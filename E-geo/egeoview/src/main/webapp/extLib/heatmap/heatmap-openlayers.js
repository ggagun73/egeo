...
OpenLayers.Layer.Heatmap = OpenLayers.Class(OpenLayers.Layer, {
	...
	tmpData: {},
	    ...
	    this.map = map;
	    this.maxbounds = new OpenLayers.Bounds() ;
	...
	setDataSet: function(obj){
	    ...
	    var minLonlat = new OpenLayers.LonLat(999999999,999999999), 
	    	maxLonlat = new OpenLayers.LonLat(0,0);
		...
		// get the pixels for all the lonlat entries
            while(dlen--){
                ...
                if(lonlat.lon<minLonlat.lon)
                	minLonlat.lon = lonlat.lon;
                if(lonlat.lat<minLonlat.lat)
                	minLonlat.lat = lonlat.lat;
                
                if(lonlat.lon>maxLonlat.lon)
                	maxLonlat.lon = lonlat.lon;
                if(lonlat.lat>maxLonlat.lat)
                	maxLonlat.lat = lonlat.lat;
        ...
        this.maxbounds.extend(minLonlat);     
        this.maxbounds.extend(maxLonlat);
