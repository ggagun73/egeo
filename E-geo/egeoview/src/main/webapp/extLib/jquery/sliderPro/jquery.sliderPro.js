		...
		resize: function() {
			...
			if ( this.settings.responsive === true ) {
				if ( ( this.settings.forceSize === 'fullWidth' || this.settings.forceSize === 'fullWindow' ) &&
					...
					this.$slider.css({ 'width': this.settings.width, 'max-width': this.settings.width, 'marginLeft': '' });
				}
			} else {
		...
		return this.each(function() {
			...
			if ( typeof $( this ).data( 'sliderPro' ) === 'undefined' ) {
				...
				if ( typeof currentInstance[ options ] === 'function' ) {
					...
					//currentInstance.selectedSlideIndex=0;
	...
	var Thumbnails = {
		...
		_thumbnailsOnResize: function() {
			...
			if ( this.settings.thumbnailsPosition === 'top' ) {
				...
				this.$slider.css({ 'paddingTop': '60px', 'paddingLeft': '', 'paddingRight': '' });
			} else if ( this.settings.thumbnailsPosition === 'bottom' ) {
		...
		initKeyboard: function() {
			...
			$( document ).on( 'keydown.' + this.uniqueId + '.' + NS, function( event ) {
				...
				if ( event.which === 37 ) {
					...
					if(that.$slider.find( '.sp-slide' ).eq( that.selectedSlideIndex ).find( '.sp-image-container a' )[0])
