...
links.Timeline.prototype.getDataRange = function (withMargin) {
    ...
    if (min && max && withMargin) {
        ...
        min = min - diff * 0.05;
      //max = max + diff * 0.05;
        max = max + diff;	// 최근 이력 가운데로 위치 조정 - Yu_mk
    }
...
links.Timeline.prototype.repaintNavigation = function () {
    ...
    if (!navBar) {
        ...
        if (showNavigation || showButtonNew) {
            ...
            navBar.style.margin = '6px 20px 0px 0px';
            
            dom.navBar = navBar;
            //frame.appendChild(navBar);
            if(navBarBool == false){
            	$("#timeBar").after(navBar);
            	navBarBool = true;
            }
        }
...
links.Timeline.prototype.selectItem = function(index) {
    ...
    if (this.items[index] != undefined) {
        var item = this.items[index],
            domItem = item.dom;
        var g2_id = $(domItem).find('.t_g2').text().trim();
        
        // 화면에 그려진 레이어를 이용한 편집이력조회 타임라인 (화면 밖은 찾지 못함)- Yu_mk
        /*var g2_id = $(".ttitle")[index].innerText.trim();
        var fid = COMMON.fn_get_editingLayer()+"."+g2_id;
        
        var editingWfsLayer = map.getLayerByName(COMMON.fn_get_editingLayer());
        for(var i=0; i<editingWfsLayer.features.length; i++){
        	if(editingWfsLayer.features[i].fid == fid){
        		alert('찾았나...');
        		break;
        	}
        }*/
    
        // 편집이력조회 타임라인 위치이동 (서버통신) - Yu_mk
        if($(".t_g2")[1].editstate == 4) COMMON.showMessage('알림 & 공간정보가 삭제된 데이터 입니다.',2000);
        else{
        	if(g2_id !== null){
        		var oParams = {
        				prefix : CONFIG.sDataHouse,
        				tables : [COMMON.fn_get_editingLayer()],
        				fields : ['FID'],
        				values : [g2_id]
        		}
        		NUTs.WFS.getFeatureByComparison(parent.serviceUrl, oParams, function(res){	
        			if(res.data.length > 0) {
        				var oFeature = res.data[0].results[0].feature;
            			
            			var oVectorLayer = editor.editLayer;
        				...
        				console.log('links.Timeline.prototype.selectItem - 편집이력 검색결과 없음');
...
links.Timeline.StepDate.prototype.getLabelMajor = function(options, date) {
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.SECOND:
            return  options.MONTHS[date.getMonth()] + " " + date.getDate() + "일 " + this.addZeros(date.getHours(), 2) + ":" + this.addZeros(date.getMinutes(), 2);
            
        case links.Timeline.StepDate.SCALE.MINUTE:
            return  date.getFullYear() + "년 " + options.MONTHS[date.getMonth()] + date.getDate() + "일 " + options.DAYS[date.getDay()] + " " ;
        ...
        case links.Timeline.StepDate.SCALE.DAY:
            return  date.getFullYear() + "년 " + options.MONTHS[date.getMonth()];
            
        case links.Timeline.StepDate.SCALE.MONTH:
            return String(date.getFullYear() + "년 ");
