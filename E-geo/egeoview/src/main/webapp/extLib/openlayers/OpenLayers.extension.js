...
OpenLayers.Events.multifeatureclick = OpenLayers.Class({
    ...
    onClick: function(evt) {
        ...
        if(features.length>0) {
        	...
        	more = this.triggerMultiEvent("multifeatureclick", {feature: features});
        }
     ...
     * {Boolean} The last listener return.
     */
    triggerMultiEvent: function(type, evt) {
        var layer = evt.feature ? evt.feature[0].layer : evt.layer,
            object = this.target.object;
        if (object instanceof OpenLayers.Map || object === layer) {
            return this.target.triggerEvent(type, evt);
        }
