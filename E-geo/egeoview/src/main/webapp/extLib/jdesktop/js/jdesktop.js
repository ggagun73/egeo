...
var nJDSK = (function(wnd,d,$){
	return{
		...
		taskbarHeight: nJDSKStyle.taskbarHeight,
	  	statusBarHeight: nJDSKStyle.statusBarHeight,
	  	 ...
	  	 */
	  	Window:function(width,height,title,toolbar,info,id,dialog,modal,fullGlass,createCallback,type,tableName){
		  	  /**
		...
		//console.log('type:' + type + ',title:' + title + ',tableName:' + tableName + ',id:' + id);  	  
		...
		if ((dialog === false) || typeof(dialog) == 'undefined')
		{
			...
			if (nJDSK.WindowList.top+25+parseInt(height) > ($(wnd).height()-nJDSK.taskbarHeight-nJDSK.statusBarHeight)){
		...
		// type=registerOnMap : 시설물 검색을 통한 대장조회
		var targetDesktopDiv = 'desktop';
		
		
		//지도가 존재할 경우에만 대장을 아래로 띄워주도록 수정... 
		if(  $("iframe").attr('id') === 'ifrMap' && targetDesktopDiv === 'desktop' ){
			nJDSK.WindowList.top = $(wnd).height()-( nJDSK.taskbarHeight+parseInt(height)+nJDSK.statusBarHeight+20);
		}
		
		if(type === 'registerOnMap' ){
			nJDSK.WindowList.left = 325;
			nJDSK.WindowList.top = $(wnd).height()-( nJDSK.taskbarHeight+parseInt(height)+nJDSK.statusBarHeight+20);
		} else {
			nJDSK.WindowList.left = 10;
		}
		...
		if ($('#win_'+id).length == 1){
		  ...
		  nJDSK.clearActive();	
	  	 ...
	  	 if (this.modal !==true)
	  	  {
	  		  $('#'+targetDesktopDiv).append(this.base);	  		 
	  	  }
	  	  else
	  	  {
    	  ...
    	  $('#win_'+id).addClass('activeWindow');
	  	  ...
	  	  /* 대장에서 TITLE 아이콘 추가 */
	  	  if (info.indexOf("/map/map.do") == -1){	  		  
	  		this.titleImg = document.createElement('div');
	  		this.titlebar.appendChild(this.titleImg);
	  		$(this.titleImg).addClass('headerBx1');	  	
	  		
	  		// 테이블 명 저장 - Yu_mk
	  		this.titleTable = document.createElement('input:hidden');
	  		this.titlebar.appendChild(this.titleTable);
	  		$(this.titleTable).attr('id', 'titleTable');
	  		$(this.titleTable).attr('name', tableName);
	  		
	  		if(type == "registerOnMap" || type == "registerOnHome")
	  			nJDSK.oBookWindow = $('#win_'+id);
	  		
		  } else {
			  // 지도의 경우 isMap 클래스 추가
			  $('#win_'+id).addClass('isMap');
			  nJDSK.oMapWindow = $('#win_'+id);
		  }
	  	  ...
	  	  if (dialog!=true)
	  	  {
	  		...
	  		if(String(id).indexOf('_') == -1){
	  		  ...
	  		  $(this.maximizeBtn).click(function(){
	  		    ...
	  		    if (($('#win_'+id).width()==$('#'+targetDesktopDiv).width()-nJDSK.windowPadding * 2)&&($('#win_'+id).height()==$('#'+targetDesktopDiv).height()-nJDSK.windowPadding * 2)){
	  		       $('#win_'+id).animate({'width':w,'height':h,'left':l,'top':t},0,function()
	  			   {
	  		    	 $('#win_'+id).removeClass('transitioner');
	  		          ...
	  		          $('#win_'+id).children('.contentarea').css({
	  		            	'width':parseInt(w.replace('px','')) - nJDSK.windowPadding * 2
	  		            });
	  		       ...
	  		       l = $('#win_'+id).css('left');
	  		       t = $('#win_'+id).css('top');
	  		       $('#win_'+id).animate({'width':($('#'+targetDesktopDiv).width()-nJDSK.windowPadding * 2),'height':($('#'+targetDesktopDiv).height()-nJDSK.windowPadding * 2),'left':0,'top':0},0,function()
	  			   {
	  		    	   $('#win_'+id).removeClass('transitioner');
	  		           $('#win_'+id).children('.contentarea').css({
	  		        	   'height':$('#win_'+id).height()-$('.titlebar').height()-2-$('#win_'+id).find('.statusbar').height()-$('#win_'+id).find('.toolbar').height()-nJDSK.windowPadding * 2
	  		           ...
	  		           $('#win_'+id).children('.contentarea').css({
	  		            	'width':($('#'+targetDesktopDiv).width()-nJDSK.windowPadding * 2)
	  		            });
	  		    ...
	  		    /* image toggle : 50% size <-> 100% size Extend */
	  		 	if($(".maximizebtn").length == 1) {
		  		    $(".maximizebtn").addClass('maximizebtn_Extend');	
		  		    $(".maximizebtn").removeClass('maximizebtn');	
	  		    }
	  		    else{
	  		    	$(".maximizebtn_Extend").addClass('maximizebtn');	
		  		    $(".maximizebtn_Extend").removeClass('maximizebtn_Extend');		  		    
	  		    }
	  	...
	  	this.close=function(){
	  		...
	  		if(String($(this)[0].w_id).indexOf('_') == -1)	// if Master(Parent) nJDSKWindow is true	  		서브의 서브가 닫혀도 같이 닫히게.. 하려면 조건이 없어야 하는데.. 
	  		{
	  			// close the master & sub window
	  			for(var i=nJDSK.WindowList.items.length-1; 0<=i; i--){
	  				if(String(nJDSK.WindowList.items[i].window_id).indexOf(String($(this)[0].w_id)) > -1){ // Master & Sub
	  					var newId = nJDSK.WindowList.items[i].window_id;
	  						
		  				$('#win_'+newId).fadeOut('fast');
		  				$('#win_'+newId).remove();
		  			  	if (this.modal==true){
		  			  		$('#Winbg_'+newId).remove();
		  			  	}
		  			  	...
		  			  	if (type == 'registerOnMap'){
		  				    parent.$('#tskbrbtn_'+newId).hide('fast',function(){
		  				    	$(this).remove();	  			  
		  				    });
		  			  	}
		  			  	else{
		  			  		 $('#tskbrbtn_'+newId).hide('fast',function(){
		  						  $(this).remove();	  			  
		  					  });
		  			  	}  
		  			  	/*unregister this window instance*/
		  			  	nJDSK.WindowList.delete_item(newId);
	  				}
	  			}
	 		}
	 		else{
		  		...
		  		/*  if (typeof(this.onBeforeClose) == 'function')
	  			  ...
	  			  //  등록 후 빨리 종료되지 않아서 폼 초기화가 되지 않음... 그래서 이걸로.. 	
	  			  $('#win_'+id).fadeOut('fast');
	  			  $('#win_'+id).remove();	  			  
/*	  			
		  		 ...
		  		 if (type == 'registerOnMap'){
					  parent.$('#tskbrbtn_'+id).hide('fast',function(){
						  $(this).remove();	  			  
					  });
		  		 }
		  		 else{
...
/*		  		  // After close event: can be used to trigger an action after the window has been closed
	  		...
	  		// 창 종료시 이전창 활성화
	  		var tempList = nJDSK.WindowList.items;
	  		if(tempList.length > 0){
	  			var tempId = tempList[tempList.length-1].window_id;
	  			var preWindow = $('#win_'+tempId);
	  			preWindow.addClass("activeWindow");
	  			if(!preWindow.is(":visible")){
	  				preWindow.show();
	  			}
	  		}
	  	  };
	  	  ...
	  	  $(this.closeBtn).click(function(){
	  		  ...
	  		  /*unregister this window instance*/
	  		  //nJDSK.WindowList.delete_item(id);
	  		  fn_update_userMapExtent(closeObject);	//시스템 종료(메인브라우저 or 맵창 닫기 or 로그아웃) 시 현재 맵 위치를 저장
	  		  closeObject.close();
	  	  });
	  	  ...
	  	  parent.document.getElementById('taskbarbuttons').appendChild(this.taskbarBtn);
	  	  $('.taskbarbutton').removeClass('activetsk');
	  	  $('#tskbrbtn_'+id).addClass('activetsk');
	  	...
	  	$(this.taskbarBtn).mouseover(function(){
	  		  /*if (($('#tskbrbtn_'+id).hasClass('activetsk')) && ($('#win_'+id).is(':visible')))
	  		  {
	  			  $('#win_'+id).hide();
	  		      if (typeof(tskbThat.onMinimize) == 'function')
	  	  ...
	  	  {
	  		  $(this.contentArea).css({
	  		    'height':$('#win_'+id).height()-$('.titlebar').height()-10  /*대상팝업창에서 문제 생김 2-> 10으로 변경 */
	  		  });
	  	  }
	  	  ...
	  	  if(type == "urlType")
	  	  {
		  	  this.frameWindow = document.createElement('iframe');
		  	  this.frameWindow.id = 'ifrMap';
		  	  this.frameWindow.src = info;
		  	  this.frameWindow.width = '100%';
		  	  this.frameWindow.height = '100%';
		  	  this.frameWindow.scrolling = 'no';
		  	  this.frameWindow.border = '0';
		  	  this.frameWindow.frameborder = '0';
		  	  this.frameWindow.marginheight = '0';
		  	  this.frameWindow.marginwidth = '0';
		  	  
		  	  $(this.contentArea).html(this.frameWindow);
	  	  }
	  	  else
	  		 $(this.contentArea).html(info);
	  	  /*
	  	...
	  	cascade: function(){
	  		...
	  		for (var i = 0; i < windowCount; i++)
	  		{
				...
				if ((wt+25+480) > ($(wnd).height()-nJDSK.taskbarHeight-nJDSK.statusBarHeight)){
					wt = 10;
				} else {
					wt+=25;
				}
	  	...
	  	init:function(){
	  		$(wnd).resize(function()
	  		{
	  			...
	  			nJDSK.desktopHeight = $(wnd).height()-nJDSK.taskbarHeight-nJDSK.statusBarHeight;
	  			$('#desktop').css({"height":(nJDSK.desktopHeight)+'px',"width":nJDSK.desktopWidth+'px', "top":nJDSK.taskbarHeight+'px'});
	  			$('#widgets').css({"height":$('#desktop').height()+'px','top':nJDSK.taskbarHeight+'px'});
	  			...
	  			if(oMapWindow) {
				  	...
				  	$(oMapContentWindow).css({'width':$(oMapWindow).width() - (nJDSK.windowPadding)});
	  			}if(oBookWindow) {
		  			oBookContentWindow = $(oBookWindow).children('.contentarea'); 
				  	$(oBookContentWindow).css({'width':$(oBookWindow).width() - (nJDSK.windowPadding)});
	  			}
	  	
	  		});	
		  
	  		//$(d).ready(function(){
  			$('#statusbar').css({"height":nJDSK.statusBarHeight+'px'});
	        ...
	        $(d).mousedown(function(ev) {
	  		...
	  		// activate menu
	  		$(d).on('mousedown', '#statusbar>ul>li>a', function() {
	        ...
	        $(d).on('mouseenter', '#statusbar>ul>li>a', function() {
	          if ($('#statusbar>ul>li>ul').is(':visible')) {
	        	  nJDSK.clearActive();
	            $(this).parents('li').addClass('activeMenu');
	            $(this).siblings('ul').show();
	          }
	        ...
	        $('a#showdesktop').click(function(e){
	        	...
	        	if ($('#selectWindowType').is(':visible'))
	        	{
	        		$('#selectWindowType').hide();
	        	}
	        	else
	        	{
	        		$('#selectWindowType').show();
	        	}
	    	...
	    	// Show/hide windows on desktop
	        /*$('a#startmenu').click(function(e){
	        	nJDSK.clearActive();
	        	var options = {};
	        	if ($('#startmenuArea').is(':visible'))
	        	{
	        		$('#startmenuArea').hide();
	        	}
	        	else
	        	{
					//$('#startmenuArea').css('width',parseInt($('#desktop_iconarea').width()-parseInt($('#startmenuArea').css('left')))); 
	        		$('#startmenuArea').show( 'blind', options, 500, startMenucallback );
	        	}
	        });*/
	        ...
	        function startMenucallback() {
	  		...
	  		$( ".draggable" ).draggable({
				...
				//containment:"desktop_iconarea",				
				helper: "clone",
				revert: "invalid",
				stop: function( event, ui ) {			
					//위치가 아이콘메뉴 영역 밖으로 나가면 실행
					var stopPos = $(this).position();
					var menuDivPos = $('#startmenuDiv').position();
				}
					
			});
			$( "#makeShoutcut" ).droppable({
				activeClass: "ui-state-default",
				drop:function(event,ui){
					var draggable = ui.draggable;
					var wind= $('.icon').filter('#'+draggable.attr('id'));			
					var title = draggable.text();
					//if(wind.length==0){												
						nJDSK.iconHelper.addIcon(draggable.attr('id'),title,'/extLib/jdesktop/images/bws_logo2k9.png',function(e){
							e.preventDefault();												
						});
					//}
					//else					
					//{
					//	alert('['+title + '] 단축아이콘이 이미 존재 합니다.' );
					//}
				}
			});
				
			// toggle icon
			$("a[name=sysico]").click(function(){	
				$("a[name=sysico]").each(function(index) {	
			...
			/*
			$(this).click(function () {
			      $(this).toggleClass("startmapview startmapviewOn");
			});*/
	  	}
	};
