...
var fn_init_map = function (_oRes, _bUserStyle, _sUserId, _sInitExtent) {
	...
	//fn_create_indexMap({layers : "법정읍면동", maxResolution : 500, projection : new OpenLayers.Projection(CONFIG.fn_get_dataHouseCrs())});
	fn_create_indexMap({layers : "BML_BADM_AS", styles : "BML_BADM_AS", maxResolution : 500, projection : new OpenLayers.Projection(CONFIG.fn_get_dataHouseCrs())});	
	...
	fn_init_divLayerTree(null, _bUserStyle);
...
var fn_create_indexMap = function (_oOptions) {
	...
	if(CONFIG.fn_get_gisEngineType() == "GeoServer"){
		...
		oIndexMapOption.styles = _oOptions.styles;
	}
...
var fn_create_map = function () {
	...
	map = new NUTs.Maps.Map('map', {  
		...
		resolutions: fn_get_resolution(nMaxResolution, nZoomLevels),
...
var fn_init_wms = function () {
	...
	if(CONFIG.fn_get_gisEngineType() === "GeoServer"){
		sThemeList = layerTool.getLayerShowList(sLayerOrder).join();
	}
		
	 
	var sDefaultThemeList = "EMPTY_LAYER";  
	sThemeList =  "Oracle:LT_C_UQ111";  // "EMPTY_LAYER";  2017-10-18 CJH
	var sSldBody;
	//debugger;
	if (!sThemeList && sThemeList.length <= 0) {
		sThemeList = ""; //sDefaultThemeList;
	...
	var oWmsOption = {
			...
			//styles : sThemeList, 2017-10-18 CJH
			
			format : sGetMapImageFormat,
			version : sGetMapVersion,
			CRS : new OpenLayers.Projection(sRequestCrs),
			transparent : true,
			//sld_body : sSldBody,    		2017-10-18 CJH
			dataHouse : CONFIG.fn_get_dataHouseName()
	};
	
	if(CONFIG.fn_get_gisEngineType() === "GeoServer") {
		//delete oWmsOption.layers;	2017-10-18 CJH
		//if(sSldBody)	2017-10-18 CJH
		//	oWmsOption.sld_body = '<?xml version="1.0" encoding="euc-kr"?>'  + sSldBody;	2017-10-18 CJH
		//oWmsOption.yx = {'EPSG:5181' : true};							2017-10-18 CJH
		//oWmsOption.styles = STYLE.fn_get_checkAllLayerNode(true);  	2017-10-18 CJH
	}
	...
	//oWmsLayer.events.register("loadend", oWmsLayer, fn_bind_fullLegendGraphic);    		2017-10-18 CJH    
...
var fn_create_favGroupTag = function(_sUserId,_sGroupName) {
	...
	for(var i in aGroupList) {
		...
		oGroupInfo = COMMON.convertToUpper(oGroupInfo);
...
var fn_create_fav = function(_oFavInfo) {
	...
	_oFavInfo = COMMON.convertToUpper(_oFavInfo);
